...
import { Timer, MapPin, Calendar, Trophy, ChevronRight, Star, LogIn, Upload } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col relative overflow-hidden font-sans">
      ...
      <style>{`
        ...
        @keyframes camera-flash {
          ...
          92% { opacity: 1; transform: scale(1.5); box-shadow: 0 0 30px 10px white; }
          94% { opacity: 0.8; transform: scale(1); }
        }
        @keyframes neon-pulse {
          0%, 100% { text-shadow: 0 0 10px #facc15, 0 0 20px #facc15; }
          50% { text-shadow: 0 0 20px #facc15, 0 0 40px #eab308, 0 0 60px #ca8a04; }
        }
        .animate-beam-slow { animation: rotate-beam 15s linear infinite; }
        .animate-beam-fast { animation: rotate-beam 10s linear infinite reverse; }
        .animate-flash { animation: camera-flash 4s infinite; }
        .neon-text { animation: neon-pulse 2s infinite alternate; }
      `}</style>
      ...
      <nav className="relative z-20 flex justify-between items-center p-6 max-w-6xl mx-auto w-full">
        <div className="flex items-center gap-2 text-yellow-400">
          <Star className="fill-yellow-400 animate-pulse" size={20} />
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center p-4 relative z-10 w-full max-w-4xl mx-auto mt-4 md:mt-0">
        ...
        {/* Big Typography Logo */}
        <div className="relative mb-8 transform -rotate-2 md:-rotate-3 animate-zoom-in">
          <h1 className="text-[5rem] md:text-[8rem] leading-[0.85] font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-white to-slate-400 drop-shadow-2xl">
            LSC
          </h1>
          <h1 className="text-[5rem] md:text-[8rem] leading-[0.85] font-black italic tracking-tighter text-yellow-400 neon-text">
            NIGHT
          </h1>
          <h1 className="text-[5rem] md:text-[8rem] leading-[0.85] font-black italic tracking-tighter text-white">
            RUN
          </h1>
        </div>

        {/* Date & Time Badge */}
        <div className="flex flex-col md:flex-row items-center gap-4 mb-8 animate-fade-in-up delay-100">
           ...
           <div className="flex items-center gap-2 text-xl font-bold italic drop-shadow-md">
             <Timer className="text-yellow-400" /> 19H LARGADA
           </div>
        </div>

        {/* Distances & Price */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl mb-10 animate-fade-in-up delay-200">
          <div className="bg-slate-900/40 backdrop-blur-md border border-slate-800 p-6 rounded-2xl flex items-center justify-between group hover:border-yellow-400/50 hover:bg-slate-900/60 transition-all duration-300 shadow-lg">
             ...
             <Trophy size={40} className="text-slate-700 group-hover:text-yellow-400 transition-colors opacity-50 group-hover:opacity-100 group-hover:scale-110 duration-300" />
          </div>
        ...
        <div className="flex flex-col items-center gap-6 animate-fade-in-up delay-300 w-full">
          ...
          >
            ...
            <div className="absolute top-0 left-[-100%] w-full h-full bg-gradient-to-r from-transparent via-white/40 to-transparent skew-x-[-25deg] group-hover:animate-[shimmer_1s_infinite]"></div>
            
            <span className="relative z-10">Fazer Inscrição Agora</span>
            <ChevronRight className="group-hover:translate-x-1 transition-transform relative z-10" strokeWidth={3} />
          </button>
          ...
          >
            <Upload size={16} className="text-yellow-500 group-hover:text-yellow-400 group-hover:animate-bounce"/>
            Já me inscrevi, enviar comprovante
          </button>
        </div>

        {/* Location Footer */}
        <div className="mt-12 flex items-center justify-center gap-2 text-slate-400 text-sm font-medium animate-fade-in-up delay-300">
